...
class Post(db.Model):
    ...
    def render_content(self):
        self.rendered_content = ContentProcessor.process_content(
            self.raw_content)
...
def feed(user_id):
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify({
            'html':
            render_template('_posts.html',
                            posts=posts_pagination.items,
                            user=user),
            'has_next':
            posts_pagination.has_next
        })
...
def like_post(user_id, post_id):
    ...
    existing_like = Like.query.filter_by(user_id=user.id,
